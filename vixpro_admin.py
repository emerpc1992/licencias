...
class LoginDialog(QDialog):
    ...
    def _finish(self, offline):
        url, key, user = self.url_in.text().strip(), self.key_in.text().strip(), self.user_in.text().strip()
    ...
    def try_login(self):
        url, key, user = self.url_in.text().strip(), self.key_in.text().strip(), self.user_in.text().strip()
        if (not self.embed.isChecked() and not self.db_in.text().strip()) or not url.startswith("http") or not key:
            self.msg.setText("Indica la URL del servidor y la contraseña (o ADMIN_KEY)"
        ...
        def work():
            try:
                r = requests.get(url.rstrip("/") + "/api/admin/ping", headers=auth_header(user, key), timeout=75)
            except requests.RequestException as e:
