...
DEFAULT_URL = ""      # cada quien pone la URL de SU servicio de Render
...
# La base de datos puede vivir DENTRO de este mismo archivo: al final hay un bloque delimitado por estas dos líneas.
# (Se arman con «+» para que las marcas no aparezcan literales en ningún otro lugar del código.)
_DB_BEGIN = "# >>> VIXPRO_DB" + "_BEGIN"
_DB_END = "# <<< VIXPRO_DB" + "_END"
_DB_RE = re.compile(re.escape(_DB_BEGIN) + r".*?" + re.escape(_DB_END), re.S)


def normalize_url(text):
    """Arregla la URL escrita a mano: agrega https://, quita espacios, «/» final y /health o /api/... pegados por error."""
    t = (text or "").strip().rstrip("/")
    if not t:
        return ""
    if not re.match(r"^https?://", t, re.I):
        t = "https://" + t
    return re.sub(r"/(health|api(/.*)?)$", "", t, flags=re.I).rstrip("/")


def auth_header(user, key):
    """Con usuario → HTTP Basic (ADMIN_USER / ADMIN_PASSWORD). Sin usuario → Bearer (ADMIN_KEY)."""
    if user:
...
ACTION_TEXT = {
    ...
    "device_forget": "Equipo olvidado", "backup": "Copia de seguridad", "restore": "Copia restaurada",
}
...
class LicenseDB:
    ...
    def __init__(self, path, embedded=False):
        """embedded=True: `path` es ESTE archivo .py y la base de datos se guarda dentro de él (bloque al final).
        embedded=False: `path` es un archivo SQLite aparte."""
        self.embedded = bool(embedded)
        self.path = Path(path)
        self.lock = threading.RLock()
        self._dirty, self._due, self._closing = False, 0.0, False
        self.on_error = None
        self.save_error = ""
        self._saver = None
        self._exp_log = {}
        if self.embedded:
            ...
            with self.lock:
                dump = self._read_embedded()
                if dump:
                    self.conn.executescript(dump)
                self.conn.executescript(SCHEMA)
                self.conn.commit()
            self._saver = threading.Thread(target=self._save_loop, daemon=True, name="vixpro-db-saver")
            self._saver.start()
        else:
    ...
    # ── base de datos embebida en el propio .py ──
    def _read_embedded(self):
        """Lee el bloque de datos que hay al final de este archivo (vacío si es la primera vez)."""
        try:
        ...
        if not m:
            return ""
        raw = re.sub(r"\s+", "", re.sub(r"(?m)^#.*$|VIXPRO_DB_B64\s*=|\"\"\"", "", m.group(0)))
        if not raw:
            return ""
    ...
    def _dump_block(self):
        ...
        blob = base64.b64encode(zlib.compress(dump.encode("utf-8"), 9)).decode()
        wrapped = "\n".join(blob[i:i + 110] for i in range(0, len(blob), 110))
        return (f"{_DB_BEGIN} (base de datos de licencias: la escribe el propio panel; NO la edites ni la subas a Render)\n"
                f'VIXPRO_DB_B64 = """\n{wrapped}\n"""\n{_DB_END}')
    ...
    def flush(self):
        """Escribe la base de datos dentro del .py (de forma atómica y comprobando que el archivo sigue siendo válido)."""
        ...
        with self.lock:
            if not self._dirty:
                return
            text = self.path.read_text(encoding="utf-8")
            new_text = self._splice(text, self._dump_block())
            compile(new_text, str(self.path), "exec")          # nunca se deja el programa dañado
    ...
    def _mark_dirty(self, structural):
        now = time.time()
        delay = 1.0 if structural else 60.0        # las simples «últimas conexiones» se guardan cada minuto como máximo
        self._due = now + delay if not self._dirty else min(self._due, now + delay)
        self._dirty = True

    def _save_loop(self):
        while not self._closing:
            time.sleep(0.5)
            if self._dirty and time.time() >= self._due:
                try:
                    self.flush()
                except Exception as e:
                    self.save_error = f"No se pudo guardar la base de datos dentro de {self.path.name}: {e}"
                    self._due = time.time() + 30
                    if self.on_error:
                        self.on_error(self.save_error)

    def _commit(self, structural=True):
        self.conn.commit()
        if self.embedded:
            self._mark_dirty(structural)

    @staticmethod
    def clean_source(path):
        """Texto de este programa SIN datos de clientes (para subir a Render / GitHub)."""
        text = Path(path).read_text(encoding="utf-8")
        empty = f'{_DB_BEGIN} (base de datos de licencias: la escribe el propio panel; NO la edites ni la subas a Render)\nVIXPRO_DB_B64 = """"""\n{_DB_END}'
        return LicenseDB._splice(text, empty)
    ...
    def restore(self, src):
        """Reemplaza TODA la base de datos por la de un archivo de copia (.db)."""
        with self.lock:
            before = self.version()
            other = sqlite3.connect(str(src))
            try:
                other.backup(self.conn)
            finally:
                other.close()
            self.conn.row_factory = sqlite3.Row
            self.conn.executescript(SCHEMA)
            # la versión nunca retrocede: si no, el servidor podría creer que su copia (más nueva) sigue al día
            self.conn.execute("INSERT INTO settings(key, value) VALUES('_version', ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",
                              (str(before),))
            self._log("admin", "restore", detail=str(src))
            self._commit()
        self.bump()
    ...
    def auto_backup(self, keep=7):
        ...
        if self.embedded:                       # la «copia» es el propio .py (lleva la base de datos dentro)
            dest = folder / f"vixpro_admin_{time.strftime('%Y%m%d')}.py"
            if dest.exists():
                return None
            self.flush()
            folder.mkdir(parents=True, exist_ok=True)
            dest.write_bytes(self.path.read_bytes())
            for old in sorted(folder.glob("vixpro_admin_*.py"))[:-keep]:
...
def create_app(admin_key=None, secret=None, admin_user=None, admin_password=None):
    ...
    admin_user = admin_user if admin_user is not None else os.environ.get("ADMIN_USER", "")
    ...
    def health():
        return jsonify({"ok": True, "service": "vixpro-licencias (relé)", "secret_configured": bool(secret),
                        "admin_configured": bool(admin_key or (admin_user and admin_password)), "admin_online": relay.admin_online(),
    ...
    def _admin_auth():
        if not request.path.startswith("/api/admin/"):
            return None
        ip = client_ip()
        if not (admin_key or (admin_user and admin_password)):
            return jsonify({"error": "Falta configurar ADMIN_USER y ADMIN_PASSWORD (o ADMIN_KEY) en el servidor"}), 503
        if limiter.count("adm-fail:" + ip, 600) >= 10:
            return jsonify({"error": "Demasiados intentos fallidos. Espera 10 minutos."}), 429
        header = request.headers.get("Authorization", "")
        ok = False
        if admin_key and header.startswith("Bearer "):
            ok = hmac.compare_digest(header[7:].encode(), admin_key.encode())
        if not ok and admin_user and admin_password and header.startswith("Basic "):
            try:
                user, _, pwd = base64.b64decode(header[6:]).decode("utf-8").partition(":")
            except Exception:
                user, pwd = "", ""
            ok = hmac.compare_digest(user.encode(), admin_user.encode()) & hmac.compare_digest(pwd.encode(), admin_password.encode())
        if not ok:
...
class RelayClient:
    ...
    def __init__(self, db, url, key, on_status=None, on_changed=None, user=""):
        self.db, self.url, self.key, self.user = db, url.rstrip("/"), key, user
    ...
    def _http(self, method, path, timeout=30, **kw):
        try:
            r = requests.request(method, self.url + "/api/admin" + path, headers=auth_header(self.user, self.key),
                                 timeout=timeout, **kw)
        except requests.RequestException as e:
            raise RelayError(f"sin conexión con el servidor ({type(e).__name__})") from e
        if r.status_code == 401:
            raise RelayAuth("usuario o contraseña incorrectos")
...
def save_config(db_path, url, key, remember, user="", embedded=True):
    try:
        CONFIG_PATH.write_text(json.dumps({"db": str(db_path), "url": url, "user": user, "embedded": bool(embedded),
                                           "key": seal(key) if remember else ""}), encoding="utf-8")
    except Exception:
        pass
...
class LoginDialog(QDialog):
    """Elige dónde vive la base de datos (por defecto DENTRO de este mismo archivo) y la conexión con el servidor."""
    ...
    def __init__(self):
        ...
        self.data = None
        cfg = load_config()
        frozen = bool(getattr(sys, "frozen", False))      # como .exe no se puede escribir dentro del programa
        ...
        lay.addWidget(sub)
        self.embed = QCheckBox("Guardar la base de datos DENTRO de este mismo archivo (.py)")
        self.embed.setChecked(bool(cfg.get("embedded", True)) and not frozen)
        self.embed.setEnabled(not frozen)
        self.embed.toggled.connect(lambda _on: self._toggle_db())
        lay.addWidget(self.embed)
        self.db_box = QWidget()
        drow = QHBoxLayout(self.db_box)
        drow.setContentsMargins(0, 0, 0, 0)
        self.db_in = QLineEdit(cfg.get("db", str(DEFAULT_DB)))
        browse = QPushButton("Examinar…")
        browse.clicked.connect(self.browse)
        drow.addWidget(QLabel("Archivo .db:"))
        drow.addWidget(self.db_in, stretch=1)
        drow.addWidget(browse)
        lay.addWidget(self.db_box)
        self.url_in = QLineEdit(cfg.get("url", DEFAULT_URL))
        self.url_in.setPlaceholderText("https://TU-SERVICIO.onrender.com  (la URL exacta que muestra Render)")
        self.user_in = QLineEdit(cfg.get("user", ""))
        self.user_in.setPlaceholderText("Usuario (ADMIN_USER de Render) — vacío si usas ADMIN_KEY")
        self.key_in = QLineEdit(unseal(cfg.get("key", "")) if cfg.get("key") else "")
        self.key_in.setEchoMode(QLineEdit.EchoMode.Password)
        self.key_in.setPlaceholderText("Contraseña (ADMIN_PASSWORD) — o la clave ADMIN_KEY")
        ...
        self.remember = QCheckBox("Recordar usuario y contraseña en este equipo (cifrados)")
    ...
    def _toggle_db(self):
        self.db_box.setVisible(not self.embed.isChecked())
    ...
    def _finish(self, offline):
        url, key, user = normalize_url(self.url_in.text()), self.key_in.text().strip(), self.user_in.text().strip()
        embedded = self.embed.isChecked()
        save_config(self.db_in.text().strip(), url, key, self.remember.isChecked(), user, embedded)
        self.data = {"db": self.db_in.text().strip(), "url": url, "key": key, "user": user, "embedded": embedded,
                     "offline": offline}
        self.accept()

    def open_local(self):
        if not self.embed.isChecked() and not self.db_in.text().strip():
            self.msg.setText("Indica dónde está (o dónde crear) el archivo de la base de datos.")
            return
        self._finish(True)

    def try_login(self):
        url, key, user = normalize_url(self.url_in.text()), self.key_in.text().strip(), self.user_in.text().strip()
        if not url or not key or (not self.embed.isChecked() and not self.db_in.text().strip()):
            self.msg.setStyleSheet(f"color:{DOWN};")
            self.msg.setText("Escribe la URL de tu servicio de Render y la contraseña (o ADMIN_KEY)"
                             + ("" if self.embed.isChecked() else ", y el archivo de la base de datos") + ".")
            return
        ...
        def work():
            try:
                r = requests.get(url + "/api/admin/ping", headers=auth_header(user, key), timeout=75)
            except requests.RequestException as e:
                raise RelayError(f"No se pudo contactar a {url} ({type(e).__name__}). Revisa que la URL esté bien escrita y que "
                                 "el servicio de Render esté en «Live».")
            if r.status_code == 404:
                try:
                    h = requests.get(url + "/health", timeout=30)
                except requests.RequestException:
                    h = None
                if h is not None and h.status_code == 200 and "vixpro" in h.text:
                    raise RelayError("El servidor responde, pero es una versión VIEJA del programa (le falta la parte del panel). "
                                     "Sube el vixpro_admin.py nuevo a GitHub y espera a que Render termine el deploy.")
                raise RelayError("Esa dirección no existe en Render (error 404). Copia la URL EXACTA de tu servicio (aparece arriba, "
                                 "debajo del nombre, y termina en .onrender.com); no uses la de ejemplo.")
            if r.status_code == 401:
                raise RelayError("Usuario o contraseña incorrectos." if user else "Clave incorrecta (¿olvidaste el usuario?).")
            if r.status_code == 503:
                raise RelayError("El servidor no tiene configurado ADMIN_USER y ADMIN_PASSWORD (o ADMIN_KEY) en Render.")
            if r.status_code >= 400:
...
class AdminWindow(QMainWindow):
    def __init__(self, db):
        ...
        self.sig.dberror.connect(lambda m: self.fail(m))
        self.db.on_error = self.sig.dberror.emit
    ...
    def build_settings_tab(self):
        ...
        self.db_path_lbl = QLabel((f"Guardada DENTRO de este mismo archivo: {self.db.path}" if self.db.embedded
                                   else f"Archivo: {self.db.path}"))
        ...
        dnote = QLabel(("La base de datos está escrita al final de este mismo archivo .py (se guarda sola tras cada cambio). "
                        "IMPORTANTE: este archivo lleva los datos de tus clientes; NO lo subas a GitHub/Render: para eso usa "
                        "«Guardar copia SIN datos». " if self.db.embedded else "")
                       + "Se hace una copia automática al día (últimas 7) en la carpeta «copias_de_seguridad». Guarda además copias "
        ...
        rs = QPushButton("♻ Restaurar copia…"); rs.clicked.connect(self.on_restore)
        cl_btn = QPushButton("📤 Guardar copia SIN datos (para Render)…"); cl_btn.setObjectName("primary"); cl_btn.clicked.connect(self.on_clean_copy)
        drow.addWidget(bk); drow.addWidget(rs); drow.addWidget(cl_btn); drow.addStretch()
        dl.addLayout(drow)
    ...
    def on_restore(self):
        path, _ = QFileDialog.getOpenFileName(self, "Restaurar copia de seguridad", "", "Base de datos (*.db)")
        if not path:
            return
        if not self.confirm("Restaurar copia", "Se REEMPLAZARÁ toda la base de datos actual por la de la copia. ¿Continuar?"):
            return
        try:
            ...
            return
        self.after_change()
        self.okay("Copia restaurada")

    def on_clean_copy(self):
        path, _ = QFileDialog.getSaveFileName(self, "Copia sin datos para Render", "vixpro_admin.py", "Python (*.py)")
        if not path:
            return
        Path(path).write_text(LicenseDB.clean_source(__file__), encoding="utf-8")
        self.okay(f"Copia sin datos guardada en {path} — esa es la que se sube a GitHub/Render")
...
def main():
    ...
    if login.exec() != QDialog.DialogCode.Accepted:
        sys.exit(0)
    d = login.data
    db = LicenseDB(Path(__file__).resolve(), embedded=True) if d["embedded"] else LicenseDB(d["db"])
    try:
        db.auto_backup()
    except Exception:
        pass
    ...
    relay = None
    if not d["offline"]:
        relay = RelayClient(db, d["url"], d["key"], on_status=win.sig.status.emit, on_changed=win.sig.changed.emit, user=d.get("user", ""))
        win.set_relay(relay)
        relay.start()
    win.show()
    code = gui.exec()
...
# >>> VIXPRO_DB_BEGIN (base de datos de licencias: la escribe el propio panel; NO la edites ni la subas a Render)
VIXPRO_DB_B64 = """"""
# <<< VIXPRO_DB_END
